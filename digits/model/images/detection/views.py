...
from .forms import ObjectDetectionModelForm
from .job import ImageObjectDetectionModelJob
from digits import frameworks
from digits import utils
from digits.config import config_value
from digits.dataset import ObjectDetectionDatasetJob
...
def new():
    """
    Return a form for a new ImageObjectDetectionModelJob
    """
    form = ObjectDetectionModelForm()
    ...
    return flask.render_template('models/images/detection/new.html',
                                 form=form,
...
def create():
    """
    Create a new ImageObjectDetectionModelJob
    ...
    for sweep in sweeps:
        ...
        job = None
        try:
            job = ImageObjectDetectionModelJob(
...
def show(job, related_jobs=None):
    ...
    return flask.render_template(
        'models/images/detection/show.html',
        job=job,
        framework_ids=[
            fw.get_id()
...
def classify_one():
    ...
    if request_wants_json():
        ...
        return flask.render_template('models/images/detection/classify_one.html',
                                     model_job=model_job,
                                     job=inference_job,
                                     image_src=image,
...
def classify_many():
    ...
    if request_wants_json():
        ...
        return flask.render_template('models/images/detection/classify_many.html',
                                     model_job=model_job,
                                     job=inference_job,
                                     paths=paths,
...
def top_n():
    ...
    return flask.render_template('models/images/detection/top_n.html',
                                 model_job=model_job,
                                 job=inference_job,
                                 results=results,
                                 )
...
def get_datasets():
    return [(j.id(), j.name()) for j in sorted(
        [j for j in scheduler.jobs.values() if isinstance(j, ObjectDetectionDatasetJob) and
...
def get_previous_networks():
    return [(j.id(), j.name()) for j in sorted(
        [j for j in scheduler.jobs.values() if isinstance(j, ImageObjectDetectionModelJob)],
        cmp=lambda x, y: cmp(y.id(), x.id())
    )
    ]
