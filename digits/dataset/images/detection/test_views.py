...
from .test_imageset_creator import create_detection_imageset
...
class BaseViewsTestWithImageset(BaseViewsTest):
    ...
    def setUpClass(cls):
        ...
        # create imageset
        cls.imageset_paths = create_detection_imageset(
            cls.imageset_folder,
...
class TestViews(BaseViewsTest, test_utils.DatasetMixin):
    ...
    def test_page_dataset_new(self):
        rv = self.app.get('/datasets/images/detection/new')
        assert rv.status_code == 200, 'page load failed with %s' % rv.status_code
        assert 'New Object Detection Dataset' in rv.data, 'unexpected page format'
...
class TestCreation(BaseViewsTestWithImageset, test_utils.DatasetMixin):
    ...
    def test_abort_explore_fail(self):
        job_id = self.create_dataset()
        self.abort_dataset(job_id)
        rv = self.app.get('/datasets/images/detection/explore?job_id=%s&db=val' % job_id)
...
class TestCreated(BaseViewsTestWithDataset, test_utils.DatasetMixin):
    ...
    def test_explore_train(self):
        rv = self.app.get('/datasets/images/detection/explore?job_id=%s&db=train' % self.dataset_id)
    ...
    def test_explore_val(self):
        rv = self.app.get('/datasets/images/detection/explore?job_id=%s&db=val' % self.dataset_id)
        if self.BACKEND == 'hdf5':
            # Not supported yet
            assert rv.status_code == 500, 'page load should have failed'
            assert 'expected backend is lmdb' in rv.data, 'unexpected page format'
