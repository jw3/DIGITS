...
@subclass
class ObjectDetectionDatasetJob(ImageDatasetJob):
    """
    A Job that creates an image dataset for a detection network
    """

    def __init__(self, **kwargs):
        super(ObjectDetectionDatasetJob, self).__init__(**kwargs)
        self.pickver_job_dataset_image_detection = PICKLE_VERSION

        self.labels_file = None

    def __setstate__(self, state):
        super(ObjectDetectionDatasetJob, self).__setstate__(state)

        if self.pickver_job_dataset_image_detection <= 1:
    ...
    def get_label_db_path(self, stage):
        ...
        """
        # detection datasets don't have label DBs
        return None
    ...
    def json_dict(self, verbose=False):
        d = super(ObjectDetectionDatasetJob, self).json_dict(verbose)
